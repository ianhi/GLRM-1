...
def pplot(As, titles):
    ...
    try:
        vmin = min(
            [A.min() for A, t in zip(As[:-1], titles) if "missing" not in t]
        )  # for pixel color reference
    except:
        vmin = As[0].min()
    try:
        vmax = max([A.max() for A, t in zip(As[:-1], titles) if "missing" not in t])
    except:
        vmax = As[0].max()
    my_dpi = 96
...
def shrinkage(a, kappa):
    ...
    try:
        return maximum(a - kappa(ones(a.shape), 0)) - maximum(
            -a - kappa * ones(a.shape), 0
        )
    except:
        return max(a - kappa, 0) - max(-1 - kappa, 0)
...
def gen_random_missing_mask(A, n_missing, return_indices=False):
    ...
    missing_idx = np.random.choice(
        np.arange(A.shape[0]), size=(n_missing, 1), replace=False
    )
    missing_idx = np.hstack(
        [missing_idx, np.random.randint(0, A.shape[1], size=(n_missing, 1))]
    )
...
def oneHotTransform(A, columns=None, max_index=None):
    ...
    return np.hstack(
        [
            A[:, i][:, None]
            if not i in columns
            else one_hot[np.abs(columns - i).argmax()]
